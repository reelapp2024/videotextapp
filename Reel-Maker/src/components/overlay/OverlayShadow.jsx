  ...
  return (
    <div className="bg-indigo-500/[0.03] p-2.5 rounded-xl border border-indigo-500/[0.06]">
      <div className="flex justify-between items-center mb-2">
        ...
        <button
          onClick={() => setField('shadowEnabled', !ov.shadowEnabled)}
          className={`text-[10px] px-2 py-0.5 rounded ${
            ov.shadowEnabled ? 'bg-green-600 text-white' : 'bg-gray-700 text-gray-400'
          }`}
        >
          {ov.shadowEnabled ? 'ON' : 'OFF'}
        </button>
      </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              min="0"
              max="20"
              value={ov.shadowBlur || 4}
              onChange={(e) => setField('shadowBlur', parseInt(e.target.value))}
              className="flex-1 h-1.5 bg-gray-600 rounded"
            />
            <span className="text-[10px] text-gray-400 w-6">{ov.shadowBlur || 4}</span>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div className="flex items-center gap-1">
              ...
              <input
                ...
                min="-20"
                max="20"
                value={ov.shadowOffsetX || 2}
                onChange={(e) => setField('shadowOffsetX', parseInt(e.target.value))}
            ...
            <div className="flex items-center gap-1">
              ...
              <input
                ...
                min="-20"
                max="20"
                value={ov.shadowOffsetY || 2}
                onChange={(e) => setField('shadowOffsetY', parseInt(e.target.value))}
                className="w-full bg-gray-700 rounded text-xs p-1 border-none"
              />
            </div>
          </div>
